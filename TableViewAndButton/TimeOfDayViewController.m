...
- (UIDatePicker *)timeOfDayPicker
{
    if (!_timeOfDayPicker) {
        ...
        [_timeOfDayPicker setDatePickerMode:UIDatePickerModeTime];
    }
...
- (void)viewDidLoad
{
    ...
    //Set default time
    NSDate *savedTime = [self.userDefaults objectForKey:@"timer_time"];
    if (savedTime) {
...
- (void)saveTime
{
    [self.userDefaults setObject:self.timeOfDayPicker.date forKey:@"timer_time"];
